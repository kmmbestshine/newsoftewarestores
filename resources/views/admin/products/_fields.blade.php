...
<div class="form-group {{ $errors->has('cov_descript') ? 'has-error' : '' }}">
    {{ Form::label('cov_descript', 'Cover cov_descript') }}
    {{ Form::textarea('cov_descript',$product->cov_descript,['class'=>'form-control border-input','placeholder'=>'Enter Cover Description']) }}
    <span class="text-danger">{{ $errors->has('cov_descript') ? $errors->first('cov_descript') : '' }}</span>
</div>
